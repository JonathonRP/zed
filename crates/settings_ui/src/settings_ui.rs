...
fn init_renderers(cx: &mut App) {
    cx.default_global::<SettingFieldRenderer>()
        ...
        .add_basic_renderer::<settings::LspDocumentColorInlayPosition>(render_dropdown)
...
pub mod test {
    ...
    #[gpui::test]
    fn registers_lsp_document_color_inlay_position_renderer(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            init_renderers(cx);
            assert!(
                cx.default_global::<SettingFieldRenderer>()
                    .renderers
                    .borrow()
                    .contains_key(&TypeId::of::<settings::LspDocumentColorInlayPosition>())
            );
        });
    }
